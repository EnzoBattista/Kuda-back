...
const NOMBRE_CLASE_DEMO = "Demo QR Acceso";
...
/** Clase + reserva para HOY, con horario ~40 min desde ahora (para probar Generar QR). */
module.exports = {
  async up(queryInterface) {
    ...
    const horaInicio = horaMasMinutos(now, 40);
    const horaFin = horaMasMinutos(now, 100);
    ...
    console.info(
      `[seeder qr-hoy-demo] Clase "${NOMBRE_CLASE_DEMO}" (id ${claseId}) para ${hoy} (${diaHoy}) ${horaInicio.slice(0, 5)}–${horaFin.slice(0, 5)}.`,
    );
    console.info("[seeder qr-hoy-demo] Reservas creadas para clientes demo. Entrá a Generar QR con ese usuario.");
  },
