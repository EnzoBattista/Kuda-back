...
const asegurarCliente = async (
  queryInterface,
  { email, dni, nombre, apellido, rolId, hash, now, genero = "masculino", telefono = "111111111" },
) => {
...
const sembrarDemoFullManual = async (queryInterface) => {
  ...
  return { vales: 0, listaEspera: { fecha: '-', cupo: 0 } };
};
...
const limpiarDatosOperativos = async (queryInterface) => {
  ...
  // Limpiar clases y profesores para testing manual estricto
  await queryInterface.sequelize.query(`DELETE FROM clases`).catch(() => {});
  await queryInterface.sequelize.query(`DELETE FROM profesor_actividad`).catch(() => {});
  await queryInterface.sequelize.query(`DELETE FROM profesores`).catch(() => {});
};
